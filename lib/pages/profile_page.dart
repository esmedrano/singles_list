...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    ...
    _loadProfileData();
    // _cleanInvalidUrls(); // Uncomment to run cleanup once, then remove
  }
  ...
  Future<void> _loadProfileData() async {
    ...
    if (metadata != null) {
      setState(() {
        ...
        _originalProfileVerified = profileVerified;
        _controller.text = metadata['intro'] ?? '';
        _originalIntro = _controller.text;
    ...
    //else {
    //   try {
    //     debugPrint('Fetching profile data for user ${user.uid} from user_ids collection at ${DateTime.now()}');
    //     final docRef = FirebaseFirestore.instance.collection('user_ids').doc(user.phoneNumber);
    //     final docSnap = await docRef.get();
    //     if (docSnap.exists) {
    //       final data = docSnap.data()!;
    //       setState(() {
    //         profileVerified = data['verified'] ?? false;
    //         _originalProfileVerified = profileVerified;
    ...
    //         _originalHeight = height;
    //         imageUrls = List<String>.from(data['imageUrls'] ?? List.generate(6, (_) => ''));
    //         _originalImageUrls = List<String>.from(imageUrls);
    //         for (int i = 0; i < imageUrls.length; i++) {
    //           if (imageUrls[i].isNotEmpty) {
    //             final file = File(imageUrls[i]);
    ...
    //         _hasUnsavedChanges = _checkForChanges();
    //       });
    //       // Convert updated_at to milliseconds for caching
    //       final cacheData = Map<String, dynamic>.from(data);
    //       if (cacheData['updated_at'] is Timestamp) {
    //         cacheData['updated_at'] = (cacheData['updated_at'] as Timestamp).millisecondsSinceEpoch;
    //       }
    //       await DatabaseHelper.instance.cacheProfileMetadata(user.uid, cacheData);
    //     }
    //   } catch (e) {
    //     debugPrint('Error loading profile from Firestore: $e at ${DateTime.now()}');
    //     ScaffoldMessenger.of(context).showSnackBar(
  ...
  bool _checkForChanges() {
    return _originalProfileVerified != profileVerified ||
        _controller.text != _originalIntro ||
...
// Version 1 is slightly broken but worth saving
/* // In profile_page.dart
import 'package:flutter/material.dart';
import 'package:image_picker/image_picker.dart';
import 'package:flutter/cupertino.dart';
...
import 'dart:typed_data';
import 'package:integra_date/scripts/verify_profile.dart'; // Import the reusable liveness capture code
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      debugPrint('Checking profile existence for user ${user.uid} in user_ids collection...');
      final docRef = FirebaseFirestore.instance.collection('user_ids').doc(user.phoneNumber);
      final docSnap = await docRef.get();
      if (!docSnap.exists) {
        debugPrint('Creating profile for user ${user.uid} with data: $data and phone number ${user.phoneNumber} at ${DateTime.now()}');
      ...
      await DatabaseHelper.instance.cacheProfileMetadata(user.uid, data);
      setState(() {
        _originalProfileVerified = profileVerified;
        _originalIntro = _controller.text;
        _originalTagsSelected = List<String>.from(tagsSelected);
  ...
  Future<void> _loadProfileData() async {
    ...
    final metadata = await DatabaseHelper.instance.getProfileMetadata(user.uid);
    if (metadata != null) {
      setState(() {
        profileVerified = metadata['verified'] ?? false;
        _originalProfileVerified = profileVerified;
  ...
  Future<void> _removeImage(int index) async {
    ...
    if (imageUrls[index].isNotEmpty) {
      try {
        if (!_isValidFirebaseStorageUrl(imageUrls[index])) {
          debugPrint('Invalid Firebase Storage URL: ${imageUrls[index]} at ${DateTime.now()}');
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Invalid image URL. Image removed from profile but not from storage.')),
          );
        } else {
          debugPrint('Deleting image from Firebase Storage: ${imageUrls[index]} at ${DateTime.now()}');
          final storageRef = FirebaseStorage.instance.refFromURL(imageUrls[index]);
  ...
  // Helper method to validate Firebase Storage URL
  bool _isValidFirebaseStorageUrl(String url) {
    if (url.trim().isEmpty) {
      debugPrint('URL is empty or contains only whitespace at ${DateTime.now()}');
      return false;
    }
    try {
      final uri = Uri.parse(url);
      final isProductionUrl = uri.scheme == 'https' &&
          uri.host.contains('firebasestorage.googleapis.com') &&
          uri.path.contains('profile_images/');
      final isEmulatorUrl = kDebugMode &&
          uri.scheme == 'http' &&
          uri.host.contains('192.168.1.153:9199') &&
          uri.path.contains('profile_images/');
      final isValid = isProductionUrl || isEmulatorUrl;
      if (!isValid) {
        debugPrint('URL validation failed: scheme=${uri.scheme}, host=${uri.host}, path=${uri.path} at ${DateTime.now()}');
      }
      return isValid;
    } catch (e) {
      debugPrint('URL parsing error: $e for URL: $url at ${DateTime.now()}');
      return false;
    }
  }
  ...
  var emptySlots = images.where((image) => image == null).length;
  if (emptySlots == 0) {
    debugPrint('No empty slots available for new images');
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('No empty slots available for new images')),
    );
    return;
  ...
  try {
    ...
    if (!kIsWeb) {
      ...
      compressedFile = File(pickedFile.path);
      debugPrint('Using original image for web platform');
    }

    debugPrint('Showing upload progress dialog...');
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (context) => const AlertDialog(
    ...
    //debugPrint('Storage emulator host: ${FirebaseStorage.instance.emulatorHost}');
    ...
    // Limit retries to avoid nonstop logging
    int retryCount = 0;
    const int maxRetries = 3;
    TaskSnapshot? snapshot;
    while (retryCount < maxRetries) {
              ...
              children: [
                ...
                Container(
                  ...
                  child: profileVerified
                      ...
                      : TextButton(
                          onPressed: _verifyFace, // Trigger liveness verification
                          style: TextButton.styleFrom(
                            minimumSize: const Size(120, 40),
                            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                            backgroundColor: const Color(0x50FFFFFF),
