...
// Wait for emulator readiness (Firestore and Storage)
async function waitForEmulator(maxAttempts = 30, interval = 2000) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      // Check Firestore
      await db.collection('test').doc('test').get();
      // Check Storage with a simpler ping
      await storage.file('test').getMetadata().catch(() => null); // Fallback to avoid early exit
      console.log('Emulators ready');
      return true;
    } catch (error) {
      console.warn(`Attempt ${attempt}/${maxAttempts}: ${error.message}`);
      if (attempt === maxAttempts) {
        console.error('Failed to connect to emulators after max attempts');
        return false;
      }
      await new Promise(resolve => setTimeout(resolve, interval));
    }
...
const imageFiles = ['oposites.jpg', 'through the dome.webp']; // Fixed typo
...
async function uploadImage(hashedId, fileName, filePath, index) {
  try {
    ...
    //console.log(`Uploaded ${fileName} (index ${index}) for hashedId ${hashedId}`);
    return `profile_images/${hashedId}/${uniqueFileName}`;
  } catch (error) {
    console.error(`Upload failed for ${fileName} (index ${index}): ${error.message}`);
    return ''; // Match old script's behavior
  }
}
